...
public:
  ...
  struct mds_info_t {
    ...
    mds_info_t() : global_id(MDS_GID_NONE), inc(0), state(STATE_STANDBY), state_seq(0),
...
public:
  ...
  */
  ...
  bool is_degraded(mds_namespace_t ns) const {   // degraded = some recovery in process.  fixes active membership and recovery_set.
    ...
    for (auto &p : mds_info) {
      if (p.second.role.ns == ns && p.second.state >= STATE_REPLAY && p.second.state <= STATE_CLIENTREPLAY) {
        return true;
      }
    }
   ...
   */
  DaemonState get_state(mds_role_t r) const {
  ...
  void get_recovery_mds_set(mds_namespace_t ns, std::set<mds_rank_t>& s) const {
    ...
    for (const auto i : fs->up) {
      ...
      if (info.role.ns == ns && info.state >= STATE_REPLAY
                             && info.state <= STATE_STOPPING) {
  ...
  void get_clientreplay_or_active_or_stopping_mds_set(mds_namespace_t ns,
    ...
    for (auto p : mds_info) {
      auto info = p.second;
      if (info.role.ns == ns && info.state >= STATE_CLIENTREPLAY
                             && info.state <= STATE_STOPPING) {
        s.insert(info.role.rank);
      }
    }
  ...
  void get_mds_set(mds_namespace_t ns,
      std::set<mds_rank_t>& s, DaemonState state) const {
    for (auto p : mds_info) {
      auto info = p.second;
  ...
  bool is_resolving(mds_namespace_t ns) const {
    ...
    return
      get_num_mds(fs, STATE_RESOLVE) > 0 &&
      get_num_mds(fs, STATE_REPLAY) == 0 &&
      fs->failed.empty();
  }
  ...
  bool is_rejoining(mds_namespace_t ns) const {  
    ...
    return 
      get_num_mds(fs, STATE_REJOIN) > 0 &&
      get_num_mds(fs, STATE_REPLAY) == 0 &&
      get_num_mds(fs, STATE_RECONNECT) == 0 &&
      get_num_mds(fs, STATE_RESOLVE) == 0 &&
      fs->failed.empty();
  }


  bool is_boot(mds_role_t r) const { return get_state(r) == STATE_BOOT; }
  bool is_creating(mds_role_t r) const { return get_state(r) == STATE_CREATING; }
  bool is_starting(mds_role_t r) const { return get_state(r) == STATE_STARTING; }
  bool is_replay(mds_role_t r) const   { return get_state(r) == STATE_REPLAY; }
  bool is_resolve(mds_role_t r) const  { return get_state(r) == STATE_RESOLVE; }
  bool is_reconnect(mds_role_t r) const { return get_state(r) == STATE_RECONNECT; }
  bool is_rejoin(mds_role_t r) const   { return get_state(r) == STATE_REJOIN; }
  bool is_clientreplay(mds_role_t r) const { return get_state(r) == STATE_CLIENTREPLAY; }
  bool is_active(mds_role_t r) const  { return get_state(r) == STATE_ACTIVE; }
  bool is_stopping(mds_role_t r) const { return get_state(r) == STATE_STOPPING; }
