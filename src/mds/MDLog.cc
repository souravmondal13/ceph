...
void MDLog::create(MDSInternalContextBase *c)
{
  ...
  journaler = new Journaler(ino, mds->get_metadata_pool(), CEPH_FS_ONDISK_MAGIC, mds->objecter,
			    logger, l_mdl_jlat,
			    &mds->timer,
                            mds->finisher);
...
void MDLog::_recovery_thread(MDSInternalContextBase *completion)
{
  ...
  if (jp.back) {
    ...
    Journaler back(jp.back, mds->get_metadata_pool(), CEPH_FS_ONDISK_MAGIC,
  ...
  Journaler *front_journal = new Journaler(jp.front, mds->get_metadata_pool(),
...
void MDLog::_reformat_journal(JournalPointer const &jp_in, Journaler *old_journal, MDSInternalContextBase *completion)
{
  ...
  Journaler *new_journal = new Journaler(jp.back, mds->get_metadata_pool(),
      CEPH_FS_ONDISK_MAGIC, mds->objecter, logger, l_mdl_jlat, &mds->timer, mds->finisher);
